...
for j=1:numAPBins       
    for i=1:numEmbryos
        ...
        if NC==12 && nc12(i,j)~=0
            ...
            % NC12
            MeanVectorAP_12(1:L12,j,i) = MeanVectorAP(nc12(i,j):nc12(i,j)+L12-1,j);
            SDVectorAP_12(1:L12,j,i) = SDVectorAP(nc12(i,j):nc12(i,j)+L12-1,j);
            NParticlesAP_12(1:L12,j,i) = NParticlesAP(nc12(i,j):nc12(i,j)+L12-1,j);
            %FractionON_Instant_12(1:L12,j,i) = NParticlesAP(nc12(i,j):nc12(i,j)+L12-1,j)./...
            %                                    NEllipsesAP(nc12(i,j):nc12(i,j)+L12-1,j);
            % NC13
            MeanVectorAP_13(1:L13,j,i) = MeanVectorAP(nc13(i,j):nc13(i,j)+L13-1,j);
            SDVectorAP_13(1:L13,j,i) = SDVectorAP(nc13(i,j):nc13(i,j)+L13-1,j);
            NParticlesAP_13(1:L13,j,i) = NParticlesAP(nc13(i,j):nc13(i,j)+L13-1,j);
            %FractionON_Instant_13(1:L13,j,i) = NParticlesAP(nc13(i,j):nc13(i,j)+L13-1,j)./...
            %                                    NEllipsesAP(nc13(i,j):nc13(i,j)+L13-1,j);
            % NC14                          
            MeanVectorAP_14(1:numFrames(i)-nc14(i,j),j,i) = MeanVectorAP(nc14(i,j):numFrames(i)-1,j);
            SDVectorAP_14(1:numFrames(i)-nc14(i,j),j,i) = SDVectorAP(nc14(i,j):numFrames(i)-1,j);
            NParticlesAP_14(1:numFrames(i)-nc14(i,j),j,i) = NParticlesAP(nc14(i,j):numFrames(i)-1,j);
...
for i=1:maxAPIndex
    for j=2:length(ElapsedTime)
        ...
        AccumulatedmRNA_SD_FractionON(j,i) = sqrt(trapz(ElapsedTime(1:j),SDVectorAP(1:j,i).^2).*FractionON(1:j,i));
        AccumulatedmRNA_SE_FractionON(j,i) = sqrt(trapz(ElapsedTime(1:j),SDVectorAP(1:j,i).^2).*FractionON(1:j,i));
    end
end
