...
% r2-far : [101]
% Predict the cases of 2 binding sites
Prediction_Run_Run_coop_compete = zeros(3, length(w_rr), length(TF));
...
for i=1:3
    ...
    for j=1:length(w_rr)
        ...
        Prediction_Run_Run_coop_compete(i,j,:) = model_6A2R_HillModel_V3_competition_RunCoop(params_6A2R_Run_coop, TF);
    end
end
...
for i=1:3
   ...
   % initialize the variables
    ...
    for j=1:length(w_rr)
        ...
        % Runt WT
        plot(APaxis(9:21), squeeze(Prediction_Run_Run_coop_compete(i,j,:)),'Color',color, 'LineWidth', 2)
%         pause
    end
