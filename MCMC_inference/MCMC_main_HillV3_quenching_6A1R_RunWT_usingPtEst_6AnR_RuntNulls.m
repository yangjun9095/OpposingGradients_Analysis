function MCMC_main_HillV3_quenching_6A1R_RunWT_usingPtEst_6AnR_RuntNulls
%% Description
...
mdl = @(TF, params) model_6A1R_HillModel_V3_quenching(params, TF);
model.modelfun = mdl;
...
%     RunNull = RuntNull(APbins_fit);
    ...
    % accepts
    names = {'K_{b}', 'K_{r}', 'w_bp', 'w_brp', 'p', 'R_{max}'};
...
for index = 1:3
    ...
    params_MCMC = [Kb, Kr, w_bp, w_brp, p, R_max];
...
%     names = {'K_{b}','\omega_{bp}','p','R_{max}'};
    names = {'log(K_{r})','\omega_{brp}'};
    ecornerplot(m,'names',names);
    
    saveas(gcf,[FigPath,filesep,'Corner_plot_Kr_logscale', constructNames{construct}  ,'.tif']); 
    saveas(gcf,[FigPath,filesep,'Corner_plot_Kr_logscale', constructNames{construct} ,'.pdf']); 
end
...
xlim([0 7])
xticks([1,2,3,4,5,6])
xticklabels({'K_{b}','K_{r}','\omega_{bp}','\omega_{brp}','p','R_{max}'})
% yticks([0 100 200 300 400])
ylim([0 1.2])
