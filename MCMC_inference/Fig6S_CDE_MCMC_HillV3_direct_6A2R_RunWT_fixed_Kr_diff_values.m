function Fig6S_CDE_MCMC_HillV3_direct_6A2R_RunWT_fixed_Kr_diff_values
%% Description
...
% constructs.
% Specifically, this script attempts to vary the K_r value inferred from
% the 6A1R cases, to x10 or x1/10 values then repeat the inference with
% increasing complexity (adding different cooperativity terms)

% Note that we will only do this for the [110] construct.
...
% Define the new K_r value for the sensitivity test
K_r_new = K_r*10;
% K_r_new = K_r*0.1;

% We will only test the [110] case, which is the 7th construct in our
% constructIndex.
for index=2 % [011, 110, and 101]
...
%     MCMCdata{index}.R_min = min(Rate_WT_forFit);
    ...
    % Define the new K_r values for the sensitivity test
    % Runt parameters
