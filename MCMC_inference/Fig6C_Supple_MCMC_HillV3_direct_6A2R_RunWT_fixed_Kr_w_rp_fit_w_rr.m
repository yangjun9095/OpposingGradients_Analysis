function Fig6C_Supple_MCMC_HillV3_direct_6A2R_RunWT_fixed_Kr_w_rp_fit_w_rr
%% Description
...
% This "Supple" script is to re-do the MCMC fitting for different K_r
% values for a sensitivity test.
...
% Define the new K_r value(s) for testing the sensitivity of the model
%K_r_new = 10*K_r;
K_r_new = 0.1*K_r;
% Here, we will run MCMC inference for each construct as we don't expect
% these w_rr/w_ho parameters to be shared across constructs.

for index=2 % [011, 110, 101]
...
%     MCMCdata{index}.R_min = min(Rate_WT_forFit);
    ...
    if index==1 % r2-new : [011]
        params_Run = [K_r_new, w_rp2, w_rp3];
    elseif index==2 % r2-close : [110]
        params_Run = [K_r_new, w_rp1, w_rp3];
    elseif index==3 % r2-far : [101]
        params_Run = [K_r_new, w_rp1, w_rp2];
    end
...
%         end
    ...
    model_mcmc = @(params) model_6A2R_HillModel_V3_direct_fixed_Kr_Bcd_RNAP_w_rp_params([params(1), 1], TF, params_fixed);
    ...
    predout = mcmcpred_V2(MCMCresults, chain(n_burn+1:end,1), [], data_pred, model_mcmc);

    %% Save the MCMC result into a structure for future usage
    MCMC_6A2R_RuntWT(index).name = '6A2R_w_rr';
    ...
    MCMC_6A2R_RuntWT(index).predout = predout;
...
FigPath = 'S:\YangJoon\Dropbox\OpposingGradientsFigures\PipelineOutput\MCMC_HillV3\Direct\seq_MCMC_inference\6A2R_prediction_from_pt_estimates_fixed_Kr\fixed_K_r_w_rp_withCoop\different_K_r_values';
...
%     plot(APaxis(APbin_start:APbin_end), output, 'Color', ColorChoice(1,:),'LineWidth', 2)
    predout = MCMC_6A2R_RuntWT(index).predout;
    out = predout;
...
%     plot(APbins, output)
    ...
    %pause
    saveas(gcf,[FigPath,filesep,'raw_fits_null_WT_95%CI_w_rr_01xKr', constructNames{construct} ,'.tif']); 
    saveas(gcf,[FigPath,filesep,'raw_fits_null_WT_95%CI_w_rr_01xKr', constructNames{construct} ,'.pdf']); 
end
