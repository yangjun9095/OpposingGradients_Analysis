function MCMC_main_HillV3_direct_6A2R_RunWT_using_Pt_ests_fixed_Kr
%% Description
...
mdl = @(TF, params) model_6A2R_HillModel_V3_direct_fixed_Kr_Bcd_RNAPparams(params, TF, params_fixed);
model.modelfun = mdl;
...
% accepts
names = {'K_{r}', 'w_rp1','w_rp2','w_rp3'};
params = cell(1, length(names));


params0 = [K_r, w_rp1, w_rp2, w_rp3];
% params0_std
...
%     elseif i==2
%         pri_mu = w_rp1;
%         pri_sig = params_Run_std(2);
%     elseif i==3
%         pri_mu = w_rp2;
%         pri_sig = params_Run_std(3);
%     elseif i==4
%         pri_mu = w_rp3;
%         pri_sig = params_Run_std(4);
%     end
...
MCMC_6A2R_RuntWT.name = 'fixed_Kr_local_w_rp_with_prior'
MCMC_6A2R_RuntWT.results = results;
MCMC_6A2R_RuntWT.chain = chain;
MCMC_6A2R_RuntWT.s2chain = s2chain;

% inferred parameters
MCMC_6A2R_RuntWT.params_inferred = params_inferred;
MCMC_6A2R_RuntWT.params_inferred_sigma = params_inferred_sigma;
...
%   
    ...
    params_temp = MCMC_6A2R_RuntWT.params_inferred;
    % combine the two sets of parameters
    if construct==3
        params_MCMC = [K_r, params_temp([2,3])];
    elseif construct==7
        params_MCMC = [K_r, params_temp([1,3])];
    elseif construct==8
        params_MCMC = [K_r, params_temp([1,2])];
    end

    
    output = model_6A2R_HillModel_V3_direct_fixed_Kr_Bcd_RNAPparams(params_MCMC, TF, params_Bcd);
    % Runt nulls
...
chain = MCMC_6A2R_RuntWT.chain;
n_burn = 0.5*n_steps;
m = [chain(n_burn+1:end,1), chain(n_burn+1:end,2), chain(n_burn+1:end,3)];
corner = figure;
%     names = {'K_{b}','\omega_{bp}','p','R_{max}'};
names = {'\omega_{rp1}', '\omega_{rp2}', '\omega_{rp3}'};
