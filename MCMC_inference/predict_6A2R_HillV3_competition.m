...
for construct = [3, 7, 8] % indices from the data.mat
    ...
    UB = [10^2, 10^2, 1, 500, 10^2, 10^2, 1, 1];
...
params = [params_bcd, 10, 10, 1, 1];
if datanum ==2
    params = [70.9320   70.5436    0.3  231.0627   10.0000   10.0000    1.0000    1.0000];
elseif datanum ==3
    params = [74.9226   77.2563    0.1388  309.2185   10.0000   10.0000    1.0000    1.0000];
elseif datanum ==1
    %params = [86.9719   48.7446    0.3287  174.8984   10.0000   10.0000    1.0000    1.0000];
    params = [86.9719  50    0.3287  174.8984   10.0000   10.0000    1.0000    1.0000];
end

Rate_null_fit = model_6A2R_HillModel_V3_competition(params, MCMCdata.xdata);
...
modelName = 'HillV3';
FilePath = 'S:\YangJoon\Dropbox\OpposingGradientsFigures\PipelineOutput\MCMC_HillV3\Competition';
MCMC_Results_1RunSite = load([FilePath,filesep,'MCMCResult_HillV3_competition','.mat'], 'MCMCResult');
...
%     params_Bcd = mean(chain_temp(n_burn+1:end,:)); % [Kb, w_bp, p, R_max];
    ...
    Prediction(i,:) = model_6A2R_HillModel_V3_competition(params_6A2R, TF_combined);
end
