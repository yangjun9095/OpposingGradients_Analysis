...
% compare the Run-dependent parameters for the Hill.V4 model, to see what varies across
% constructs.
% Hill.V4 model represents a model from Hill.V4 with an assumption that the
% Run site(s) is(are) occupied all the time, meaning the K_{r}<<1
...
%     clf
    ...
    n_burn = 0.5*n_steps;
%     m = [chain(n_burn+1:end,1), chain(n_burn+1:end,1)];
    corner = figure;
%     names = {'K_{b}','\omega_{bp}','p','R_{max}'};
%     names = {'\omega_{rp}'};
%     ecornerplot(m,'names',names);
    histogram(chain(n_burn+1:end,1),50,'Normalization','probability')
    xlabel('\omega_{rp}')
    ylabel('frequency')
    set(gca,'fontsize',12)
    StandardFigure(gcf,gca)
...
for index = 1:3
    ...
    errorbar(1, params_inferred, params_inferred_std,'o','LineWidth',2,'Color',ColorChoice(construct,:))
end

xlim([0 3])
xticks([1])
xticklabels({'\omega_{rp}'})
...
% % save the plots
saveas(gcf,[FigPath,filesep, 'MCMC_6A1R_HillV3_Run_parameters_point_est_BcdParams','.tif']);
saveas(gcf,[FigPath,filesep, 'MCMC_6A1R_HillV3_Run_parameters_point_est_BcdParams','.pdf']);
...
errorbar(1, params_CV, params_CV_error,'o','LineWidth',2)
