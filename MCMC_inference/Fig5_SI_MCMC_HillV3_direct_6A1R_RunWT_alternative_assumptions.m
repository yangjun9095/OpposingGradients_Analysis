function Fig5_SI_MCMC_direct_6A1R_RunWT_alternative_assumptions
%% Description
...
data_pred = {MCMCdata.APdata; MCMCdata.APdata; MCMCdata.APdata};
...
% % Define the MCMCresults for Kr and w_rp only for each dataset
MCMCresults = results;
MCMCresults.parind = [1;2];
MCMCresults.local = [1,2];
MCMCresults.theta = [results.theta(1); results.theta(index+1)];

out = mcmcpred(MCMCresults, chain(n_burn+1:end,:), [], data_pred, model_mcmc);
...
for index = 1:3
    ...
    % MCMCpred
    nn = (size(out.predlims{index}{1},1)+1)/2;
    plimi = out.predlims{index}{1};
    yl = plimi(3,:);
    yu = plimi(2*nn-3,:);
    %plot(APaxis(APbin_start:APbin_end), output)
    shadedErrorBar(APbins, output, [yu;yl],'lineProps','-k')%['color', ColorChoice(1,:),'LineWidth', 2])
    xlim([0.2 0.5])
