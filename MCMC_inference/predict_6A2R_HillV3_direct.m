...
modelName = 'HillV3';
FilePath = 'S:\YangJoon\Dropbox\OpposingGradientsFigures\PipelineOutput\MCMC_HillV3\Direct';
load([FilePath,filesep,'MCMCresult_',modelName,'_','direct_','params_1RunSite','.mat'],'MCMCResult', 'params_MCMC', 'params_MCMC_std');
...
Prediction_Run_Run_coop = zeros(3, length(w_rr), 82);
...
for i=1:3
   ...
   % initialize the variables
    ...
    for j=1:length(w_rr)
        ...
        % Prediction
        % Runt null
        plot(APaxis, squeeze(Prediction_Run_Run_coop(i,j,1:41)),'Color',ColorChoice(4,:), 'LineWidth', 2)
        % Runt WT
        plot(APaxis, squeeze(Prediction_Run_Run_coop(i,j,42:end)),'Color',color, 'LineWidth', 2)
        pause
    end
...
%   %Save the plot
    saveas(gcf,[FigPath,filesep,'Direct',filesep,'Prediction_HillV3_Run_Run_coop_',constructNames{construct},'.tif']); 
    saveas(gcf,[FigPath,filesep,'Direct',filesep,'Prediction_HillV3_Run_Run_coop_',constructNames{construct},'.pdf']);
    pause
end
...
% mnodel_6A2R_HillModelV3_RunCoop.m
% w > 1 : stronger repression than independent Runt effect.
...
for i=1:3
   ...
   % initialize the variables
    ...
    for j=1:length(w_rr)
        ...
        % Prediction
        % Runt null
        plot(APaxis, squeeze(Prediction_higher_coop(i,j,1:41)),'Color',ColorChoice(4,:), 'LineWidth', 2)
        % Runt WT
        plot(APaxis, squeeze(Prediction_higher_coop(i,j,42:end)),'Color',ColorChoice(1,:), 'LineWidth', 2)
...
%   %Save the plot
%     saveas(gcf,[FigPath, filesep, 'Direct', filesep,'Prediction_HillV3_HighCoop_',constructNames{construct},'.tif']); 
%     saveas(gcf,[FigPath,filesep, 'Direct', filesep, 'Prediction_HillV3_HighCoop_',constructNames{construct},'.pdf']);
    pause
end
