function [models,logP,frac_accept]=gwmcmc(minit,logPfuns,mccount,varargin)
...
N_total = mccount; %Total number of steps
...
progress(0,0,0)
totcount=Nwalkers;
N_accept = zeros(1,Nwalkers);
w = waitbar(0,strcat('Step #0 of ',num2str(Nkeep))); %Progress bar

for row=1:Nkeep
    for jj=1:p.ThinChain
        waitbar(row/Nkeep,w,strcat('Step # ',num2str(row),' of ',num2str(Nkeep))); %Update progress bar
...
close(w);
progress(1,0,0);
frac_accept = sum(N_accept) / N_total;
if p.BurnIn>0
