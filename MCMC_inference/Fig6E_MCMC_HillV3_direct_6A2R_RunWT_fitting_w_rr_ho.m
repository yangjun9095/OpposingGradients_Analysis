function Fig6E_MCMC_HillV3_direct_6A2R_RunWT_fitting_w_rr_ho
%% Description
...
% Note: This script is for fitting both w_rr and w_ho. We perform MCMC
% inference for each construct independently. 
% For AIC/BIC calculation, we might need to sum up all three constructs (or
% average).
...
%% Here, we will run MCMC inference for each construct as we don't expect these w_rr/w_ho parameters to be shared across constructs.

for index=1:3 % [011, 110, 101]
...
%         end
        ...
        if (index==1) | (index==3) % for [011] and [101] constructs, fix the w_rr values to be 1
            if i==1 % w_rr
                targetflag = 0; % fix the parameter
            end
        end
    ...
    % For [011] and [101] constructs, add 1s for the w_rr chain
    if (index==1) | (index==3)
        chain = [ones(n_steps,1), chain];
    end

    % compute the mean and std of inferred parameters
    for k=1:2
    ...
    model_mcmc = @(params) model_6A2R_HillModel_V3_direct_fixed_Kr_Bcd_RNAP_w_rp_params(params, TF, params_fixed);
...
FigPath = 'S:\YangJoon\Dropbox\OpposingGradientsFigures\PipelineOutput\MCMC_HillV3\Direct\seq_MCMC_inference\6A2R_prediction_from_pt_estimates_fixed_Kr\fixed_K_r_w_rp_withCoop\both_Run_Run_higher_order';
