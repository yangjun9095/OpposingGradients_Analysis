...
%         end
    ...
    model_mcmc = @(params) model_6A2R_HillModel_V3_direct_fixed_Kr_Bcd_RNAP_w_rp_params([params(1), params(2)], TF, params_fixed);

    MCMCresults = results;
    MCMCresults.parind = [1,2];
    MCMCresults.local = [1,2];
    MCMCresults.theta = [results.theta(1),results.theta(1)];
    ...
    MCMC_6A2R_RuntWT(index).name = '6A2R_w_rr_w_ho';
...
%     plot(APbins, output)
    ...
    %pause
    saveas(gcf,[FigPath,filesep,'raw_fits_null_WT_95%CI_w_rr_w_ho_01xKr', constructNames{construct} ,'.tif']); 
    saveas(gcf,[FigPath,filesep,'raw_fits_null_WT_95%CI_w_rr_w_ho_01xKr', constructNames{construct} ,'.pdf']); 
end
