...
results = [];
[results,~,~,~]=mcmcrun(model,MCMCdata(index),params,options,results);
[results,~,~,~]=mcmcrun(model,MCMCdata(index),params,options,results);
%[results,~,~,~]=mcmcrun(model,MCMCdata,params,options,results);
[results,chain,s2chain,~]=mcmcrun(model,MCMCdata(index),params,options,results);
...
output = model_6A1R_HillModel_V3(params_inferred, MCMCdata(index).xdata);
...
% Runt Null
plot(APaxis(APbins_fit), output(1:length(APbins_fit)),'Color',ColorChoice(4,:),'LineWidth',2)
% Runt WT
plot(APaxis(APbins_fit), output(1+length(APbins_fit):end),'Color',ColorChoice(1,:),'LineWidth',2)
...
%% Save the MCMC results into one structure
modelName = 'HillV3';

% MCMCResult = struct;
% MCMCresult = load([FilePath, filesep, 'MCMCresult_',modelName,'_','001','.mat'])
% k=1; % counter
% for construct = [2,5,6]
%     MCMCResult(k) = load([FilePath, filesep, 'MCMCresult_',modelName,'_',constructNames{construct},'.mat'], 'chain', 'results');
% %     MCMCResult(k).name = data(construct).constructName;
%     % update the counter
%     k=k+1;
% end
clear MCMCResult

MCMCResult(1) = load([FilePath, filesep, 'MCMCresult_',modelName,'_',constructNames{2},'.mat'], 'chain', 'results');
MCMCResult(2) = load([FilePath, filesep, 'MCMCresult_',modelName,'_',constructNames{5},'.mat'], 'chain', 'results');
MCMCResult(3) = load([FilePath, filesep, 'MCMCresult_',modelName,'_',constructNames{6},'.mat'], 'chain', 'results');
...
% Fits
range = [0.1 1 5 10 50 100]; % range of K_{r}
% Using the colorbar for range of K_{r} values
cmap = colormap('viridis');
caxis([0.1 100])
for i=1:length(range)

    params_inferred(2) = range(i);
    cIndex = floor((1-(100-range(i))/(100-0.1))*256);
    if cIndex ==0
        cIndex = cIndex + 1;
    end
    ...
    % Runt Null
    plot(APaxis(APbins_fit), output(1:length(APbins_fit)),'Color',ColorChoice(4,:),'LineWidth',1)
    % Runt WT
    plot(APaxis(APbins_fit), output(1+length(APbins_fit):end),'Color',color,'LineWidth',1)
