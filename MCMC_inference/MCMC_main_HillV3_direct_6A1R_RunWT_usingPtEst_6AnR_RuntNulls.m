function MCMC_main_HillV3_direct_6A1R_RunWT_usingPtEst_6AnR_RuntNulls
%% Description
...
for index = 1:3
    ...
    errorbar(1:2, [log10(params_inferred(1)), params_inferred(2)],...
                    [log10(params_inferred_std(1)), params_inferred_std(2)],'o','LineWidth',2,'Color',ColorChoice(construct,:))
end
...
%% CV - log scale for the parameters
params_inferred_all = [];
params_inferred_error_all = [];
...
% calculate the mean and std of parameters "over constructs".
params_null_mean = mean(log10(params_inferred_null));
params_null_std = std(log10(params_inferred_std_null));

params_mean = mean(log10(params_inferred_all));
params_std = std(log10(params_inferred_all));
% params_error = sqrt(sum(params_inferred_error_all.^2)/length(data))
n_boots = 100;
params_null_std_boostrap = bootstrp(n_boots, @std, log10(params_inferred_null));
parms_std_boostrap = bootstrp(n_boots, @std, log10(params_inferred_all));
...
% save the plots
% saveas(gcf,[FigPath,filesep, 'CV_parameters_log','.tif']);
% saveas(gcf,[FigPath,filesep, 'CV_parameters_log','.pdf']);
