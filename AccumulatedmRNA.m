function [AccumulatedmRNA, AccumulatedmRNA_SD, AccumulatedmRNA_SE,...
            AccumulatedmRNA_individual, AccumulatedmRNA_SD_individual] = AccumulatedmRNA(DataType, MinParticles, varargin)
...
for k=1:numEmbryos
    % Looop over all AP bins % Here, I need to consider the APbinArea, like
    % small AP bins should be ignored, since it'll only add bias.
    for i=1:nAPbins
        for j=2:length(ElapsedTime)
            ...
            AccumulatedmRNA_individual(j,i,k) = trapz(ElapsedTime(1:j),MeanVectorAP(1:j,i,k).*NParticlesAP(1:j,i,k)); 
            AccumulatedmRNA_SD_individual(j,i,k) = sqrt(trapz(ElapsedTime(1:j),SDVectorAP(1:j,i,k).^2 .*NParticlesAP(1:j,i,k)));
