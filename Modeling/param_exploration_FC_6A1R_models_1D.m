...
%% Part1. 1-D, Fold-Change at different extremes
%% 1) Bicoid saturating regime
clear FC_quenching
clear FC_direct
...
% fitted parameters :
params_direct = [99.9970    5.0000    1.0198    9.9183    0.2000    0.0003  366.4926]; % fitted with Model_V1
...
%                         FC_competition(i,j,k,l,m,n,:) = model_FC_6A1R_competition_V1(Bcd, Runt, params_compete);
                        FC_quenching = model_FC_6A1R_quenching_V1(Bcd, Runt, params_quench);
                        plot(APaxis, FC_quenching, 'Color', c(a,:))
                        a=a+1;
            ...
            for l=1:length(W_bp)
                for m=1:length(P)
                    for n=1:length(W_brp) % for the repression terms, w_rp, w_brp, w_br
                        ...
                        plot(APaxis, FC_quenching(i,j,k,l,m,n,:),'Color',c(a,:))
                        xlim([0.2 0.6])
                        ylim([0 1.2])
                    end
                end
