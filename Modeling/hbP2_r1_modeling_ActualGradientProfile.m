% hbP2 + 1 Runt binding site construct modeling with actual Bcd and Runt
% gradient.
...
% it's from Jonathan.
Bcd = load('E:\YangJoon\LivemRNA\Data\Dropbox\OpposingGradient\Bicoid.mat')
...
Runt = load('E:\YangJoon\LivemRNA\Data\Dropbox\OpposingGradient\Runt-1min-200Hz-Male-Averaged.mat');
...
%% Case0. hb P2, without any Runt binding site
P = [0.1:0.1:1,2:10];
wA = [1,exp(1),exp(2),exp(3),exp(4),exp(5)]; % interaction term between Bcd molecules
        ...
        for k=1:length(wAP)
            for m=1:length(BcdScale)
                for n=1:length(RuntScale)
                    ...
                    PartitionFunction = 1/wA(j)*(1+BcdScale(m)*Bcd*wA(j)).^6 + (1-1/wA(j))+...
                                        P(i)*(1+BcdScale(m)*Bcd*wA(j)*wAP(k)).^6 + P(i)*(1-1/wA(j));
                    RNAP_bound = P(i)*(1+BcdScale(m)*Bcd*wA(j)*wAP(k)).^6 + P(i)*(1-1/wA(j));
...
Maximum_r0 = Maximum;
BoundaryPosition_r0 = BoundaryPosition;
Slope_r0 = Slope;
DynamicRange_r0 = DynamicRange;
