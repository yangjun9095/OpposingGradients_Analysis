...
Z = 1 + p + b.^6 + r1 + r2 +...
    b.^6.*(r1 + r2 + p*w_bp) + r1*p*omega_rp1 + r2*p*omega_rp2 + r1.*r2.*omega_rr +...
    r1.*r2*p*omega_rp1*omega_rp2.*omega_rr.*omega_ho + ...
    b.^6*p*w_bp.*(r1*omega_rp1 + r2*omega_rp2) + b.^6.*r1.*r2.*omega_rr +...
    b.^6.*r1.*r2*p*w_bp*omega_rp1*omega_rp2.*omega_rr.*omega_ho;

Z_p = p +  b.^6*p*w_bp + r1*p*omega_rp1 + r2*p*omega_rp2 +...
        r1.*r2*p*omega_rp1*omega_rp2.*omega_rr +...
        b.^6*p*w_bp.*(r1*omega_rp1 + r2*omega_rp2)+...
         b.^6.*r1.*r2*p*w_bp*omega_rp1*omega_rp2.*omega_rr.*omega_ho;
