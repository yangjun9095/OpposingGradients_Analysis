function [Rate] = model_6A2R_HillModel_V3_quenching_fixed_Kr_Bcd_RNAP_w_rp_params(params,TF, params_fixed) 
...
K_r = params_fixed(5);
omega_brp1 = params_fixed(6);
omega_brp2 = params_fixed(7);
...
Z = 1 + p + b.^6 + r1 + r2 +...
    b.^6.*(r1 + r2 + p*w_bp) + r1*p + r2*p + r1.*r2.*omega_rr +...
    r1.*r2*p*omega_rr.*omega_ho + ...
    b.^6*p*w_bp.*(r1*omega_brp1 + r2*omega_brp2) + b.^6.*r1.*r2.*omega_rr +...
    b.^6.*r1.*r2*p*w_bp*omega_brp1*omega_brp2.*omega_rr.*omega_ho;

Z_p = p +  b.^6*p*w_bp + r1*p + r2*p +...
        r1.*r2*p.*omega_rr +...
        b.^6*p*w_bp.*(r1*omega_brp1 + r2*omega_brp2)+...
         b.^6.*r1.*r2*p*w_bp*omega_brp1*omega_brp2.*omega_rr.*omega_ho;
