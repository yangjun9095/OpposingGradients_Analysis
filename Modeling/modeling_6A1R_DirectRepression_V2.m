...
mdlo = @model_6A1R_direct_repression_V2;
...
[params_fit_null,~,Res_null,~,~,~,Jacobian_null] =...
                    lsqcurvefit(mdlo, params0, TF(fitRange,1:2:end), Rate_null(fitRange), lb, ub, optimoptions);
...
% fun = @(params)model_6A1R_direct_repression_V2(TF(fitRange,:),params1)- Rate(fitRange);
                    
[params_fit,~,Res,~,~,~,Jacobian] =...
                    lsqcurvefit(mdlo, params1, TF(fitRange,1:2), Rate(fitRange), lb, ub, optimoptions);

%% Calculate the CI of the model fit and parameters
% First, get the CI for the fitted parameters
CI_null = nlparci(params_fit_null, Res_null, 'jacobian', Jacobian_null);

CI = nlparci(params_fit, Res, 'jacobian', Jacobian);

% Second, calculate the CI for the predicted fit using nlpredci
[Ypred_null,delta_null] = nlpredci(mdl,TF(:,1:2:end),params_fit_null,Res_null,'Jacobian',full(Jacobian_null));

[Ypred,delta] = nlpredci(mdl,TF(:,1:2),params_fit,Res,'Jacobian',full(Jacobian));
