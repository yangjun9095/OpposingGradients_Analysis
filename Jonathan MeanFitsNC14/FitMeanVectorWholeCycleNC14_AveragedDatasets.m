function FitMeanVectorWholeCycleNC14_AveragedDatasets(DataType,varargin)

% Last updated on: 4/16/2020 by Jonathan Liu
% Edited by Yang Joon Kim on 06/04/2020 to apply this to 

%
% Edited by Yang Joon Kim on 06/02/2020 for fitting to the averaged dataset
...
% Note that this one is modified to look for AveragedDatasets.
...
%Default settings
ncwindow = [1, 25]; %Time of fitting for nc14, [min]
...
%Load the AveragedDatasets                                    
data = load([filepath,filesep,DataType,'.mat']);
%APDivision = load([DropboxFolder,filesep,Prefix,'\APDivision.mat']);
...
% Default parameters
APResolution = 0.025; %2.5% of the embryo length
bins = 1/APResolution + 1;
...
% extract info for time and APbins

% Note that the time points are from the beginning
...
% For each AP bin, define the fields, then fit the model
for AP = 1:N_bins
    ...
    if nc14~=0
        t = data.ElapsedTime - data.ElapsedTime(nc14);
    end
    ...
    FramesToKeep = and(t(Frame_raw) >  firstnc14time, t(Frame_raw) <  lastnc14time);
    Frame = Frame_raw(FramesToKeep);
    % We keep this "Frame_window" for capping the time window before time interpolation
    Frame_window = Frame_raw(FramesToKeep); 
    ...
    t_interp = t(nc14):(dt/t_fold):t(end)+ (dt/t_fold); %Fit time vector
...
loc = filepath;
filename = 'MeanWholeCycleFitsNC14_';
save([loc,filename,DataType,'.mat'],'FitResults','ResultsPlot','DataType');
...
ub = [200, 20, 20, 30, 50, 1e7]; %upper bound
