function FitMeanAPInitialPhaseV2(varargin)
...
while (cc~=13)
    ...
    if APDivision(CurrentNC,i)
        ...
        if (sum(NParticlesAP(FrameWindow,i)>=MinParticles)>=MinTimePoints)
            ...
            elseif CurrentNC==14 || CurrentNC==13
...
%             ElapsedTime(eval(['nc',num2str(nc)]))])
    ...
    elseif (ct~=0)&(cc=='S')
        if ~exist([DropboxFolder,filesep,Prefix,filesep,'InitialPhaseFit_snapshots'])
            mkdir([DropboxFolder,filesep,Prefix,filesep,'InitialPhaseFit_snapshots'])
        end
        FigPath = [DropboxFolder,filesep,Prefix,filesep,'InitialPhaseFit_snapshots'];
        % Save the figures as .tif and .pdf
        StandardFigure(FitFigure,FitFigure.CurrentAxes)
        saveas(FitFigure,[FigPath,filesep, 'InitialPhaseFit_AP=', num2str(APbinID(i)*100),'%' , '_NC',num2str(CurrentNC) , '.tif']); 
        saveas(FitFigure,[FigPath,filesep, 'InitialPhaseFit_AP=', num2str(APbinID(i)*100),'%' , '_NC',num2str(CurrentNC) , '.pdf']); 
        display('Plot for the fitted slope and raw data is saved')
        
    %Save
    elseif (ct~=0)&(cc=='e')
        save([DropboxFolder,filesep,Prefix,filesep,'MeanFitsInitialPhase.mat'],...
        'FitResults')
    display('MeanFitsInitialPhase.mat saved')
